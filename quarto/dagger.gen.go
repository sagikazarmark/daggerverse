...
func (r *Quarto) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Ctr *Container
	}
...
func (r *Renderer) UnmarshalJSON(bs []byte) error {
	var concrete struct {
		Ctr  *Container
		Args []string
	}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Quarto":
		...
		case "Container":
			...
			return (*Quarto).Container(&parent), nil
		case "Render":
			var parent Quarto
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			var siteUrl string
			if inputArgs["siteUrl"] != nil {
				err = json.Unmarshal([]byte(inputArgs["siteUrl"]), &siteUrl)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg siteUrl", err))
				}
			}
			return (*Quarto).Render(&parent, ctx, source, input, siteUrl), nil
		case "":
			var parent Quarto
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return New(version, image, container), nil
		default:
	...
	case "Renderer":
		...
		case "Directory":
			...
			return (*Renderer).Directory(&parent), nil
		case "File":
			var parent Renderer
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Renderer).File(&parent, name), nil
		default:
			return nil, fmt.Errorf("unknown function %s", fnName)
		}
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Quarto").
					WithFunction(
						dag.Function("Container",
							dag.TypeDef().WithObject("Container"))).
					WithFunction(
						dag.Function("Render",
							dag.TypeDef().WithObject("Renderer")).
							WithDescription("Render files or projects to various document types.").
							WithArg("source", dag.TypeDef().WithObject("Directory"), FunctionWithArgOpts{Description: "Quarto source directory."}).
							WithArg("input", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Input to render within the project."}).
							WithArg("siteUrl", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Override site-url for website or book output."})).
					WithConstructor(
						dag.Function("New",
							dag.TypeDef().WithObject("Quarto")).
							WithArg("version", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Version (image tag) to use from the official image repository as a base container."}).
							WithArg("image", dag.TypeDef().WithKind(StringKind).WithOptional(true), FunctionWithArgOpts{Description: "Custom image reference in \"repository:tag\" format to use as a base container."}).
							WithArg("container", dag.TypeDef().WithObject("Container").WithOptional(true), FunctionWithArgOpts{Description: "Custom container to use as a base container."}))).
			WithObject(
				dag.TypeDef().WithObject("Renderer").
					WithFunction(
						dag.Function("Directory",
							dag.TypeDef().WithObject("Directory")).
							WithDescription("Get the output directory after rendering.")).
					WithFunction(
						dag.Function("File",
							dag.TypeDef().WithObject("Directory")).
							WithDescription("Get the output file after rendering.").
